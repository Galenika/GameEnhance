...
void CConfig::Setup()
{
	...
	SetupValue( Vars.Ragebot.Hitbox, 8, strenc( "Ragebot" ), strenc( "Hitbox" ) );
	...
	SetupValue( Vars.Legitbot.Aimbot.Hitbox, 8, strenc( "Legitbot" ), strenc( "Hitbox" ) );
...
void CConfig::DoSave()
{
	...
	{
		if (Vars.Menu.ConfigIndex == 0)
		{
			folder = std::string(path) + strenc("\\TirRun\\");
			file = std::string(path) + strenc("\\TirRun\\rage.ini");
		}
		if (Vars.Menu.ConfigIndex == 1)
		{
			folder = std::string(path) + strenc("\\TirRun\\");
			file = std::string(path) + strenc("\\TirRun\\legit.ini");
		}
		if (Vars.Menu.ConfigIndex == 2)
		{
			folder = std::string(path) + strenc("\\TirRun\\");
			file = std::string(path) + strenc("\\TirRun\\faceit.ini");
		}
	}
...
void CConfig::DoLoad()
{
	...
	if ( SUCCEEDED( SHGetFolderPath( NULL, CSIDL_APPDATA, NULL, 0, path ) ) )
	{
		...
		if (Vars.Menu.ConfigIndex == 2 || Vars.Menu.LoadFaceit)
		{
			folder = std::string(path) + strenc("\\TirRun\\");
			file = std::string(path) + strenc("\\TirRun\\faceit.ini");
		}
