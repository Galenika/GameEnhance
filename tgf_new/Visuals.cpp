...
void ASUS(bool state)
{
	...
	for (MaterialHandle_t i = I::MaterialSystem->FirstMaterial(); i != I::MaterialSystem->InvalidMaterial(); i = I::MaterialSystem->NextMaterial(i))
	{
		...
		if (strstr(pMaterial->GetTextureGroupName(), charenc("World")))
		{
			if (state)
			{
				pMaterial->AlphaModulate(Vars.Visuals.ASUSFactor);
...
void CVisuals::Run()
{
	...
	else if (!Vars.Visuals.CrosshairOn)
	{
...
void CVisuals::Watermark()
{
	...
	D::DrawString( F::Watermark, 50, 110, Color( 255,255,255,255 ), FONT_LEFT, "%s", charenc( "´Watermark" ) );
}
