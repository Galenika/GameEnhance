...
void CGUI::Draw()
{
	...
	if (ShouldDrawCursor)
	{
		...
		Render::PolygonOutline(3, MouseVt, Color(55, 102, 255, 255), Color(55, 102, 255, 255));
	}
}
...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	Render::Outline(window->m_x, window->m_y, window->m_iWidth, window->m_iHeight, Color(18, 18, 18, 255));
	Render::GradientV(window->m_x + 2, window->m_y + 2, window->m_iWidth - 4, 26, Color(90, 0, 90, 255), Color(18, 18, 18, 255));
	Render::Clear(window->m_x + 2, window->m_y + 2 + 26, window->m_iWidth - 4, window->m_iHeight - 4 - 26, Color(18, 18, 18, 255));
	Render::Outline(window->m_x + 1, window->m_y + 1, window->m_iWidth - 2, window->m_iHeight - 2, Color(18, 18, 18, 255));
	// Main Window
	Render::Outline(window->m_x, window->m_y, window->m_iWidth, window->m_iHeight, Color(18, 18, 18, 255));
	Render::GradientV(window->m_x + 2, window->m_y + 2, window->m_iWidth - 4, 26, Color(18, 18, 18, 255), Color(18, 18, 18, 255));
	Render::Clear(window->m_x + 2, window->m_y + 2 + 26, window->m_iWidth - 4, window->m_iHeight - 4 - 26, Color(18, 18, 18, 255));
	Render::Outline(window->m_x + 1, window->m_y + 1, window->m_iWidth - 2, window->m_iHeight - 2, Color(18, 18, 18, 255));
	Render::Text(window->m_x + 470, window->m_y + 8, Color(218, 218, 218, 255), Render::Fonts::MenuBold, window->Title.c_str());
	//Inner
	Render::Outline(window->m_x + 7, window->m_y + 1 + 26, window->m_iWidth - 4 - 10, window->m_iHeight - 2 - 6 - 26, Color(18, 18, 18, 255));
	Render::Clear(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, window->m_iHeight - 2 - 8 - 26, Color(18, 18, 18, 255));

	//Tab
	Render::GradientV(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, 29, Color(18, 18, 18, 255), Color(18, 18, 18, 255));
	int TabCount = window->Tabs.size();
	if (TabCount) // If there are some tabs
	{
		int TabSize = (window->m_iWidth - 4 - 12) / TabCount;
		for (int i = 0; i < TabCount; i++)
		{
			...
			{
				Render::GradientV(window->m_x + 8 + (i*TabSize), window->m_y + 1 + 27, TabSize, 29, Color(18, 18, 18, 255), Color(18, 18, 18, 255));
			}
			else if (IsMouseInRegion(TabArea))
			{
				Render::GradientV(window->m_x + 8 + (i*TabSize), window->m_y + 1 + 27, TabSize, 29, Color(18, 18, 18, 255), Color(18, 18, 18, 255));
			}
			RECT TextSize = Render::GetTextSize(Render::Fonts::Untitled1, tab->Title.c_str());
			Render::Text(TabArea.left + (TabSize / 2) - (TextSize.right / 9), TabArea.top + 8, Color(195, 195, 195, 255), Render::Fonts::Untitled1, tab->Title.c_str());
		}
	}
