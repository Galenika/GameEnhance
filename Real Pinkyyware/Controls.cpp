...
void CCheckBox::Draw(bool hover)
{
	...
	if (hover)
	{
		if (Checked)
		{
			Render::Clear(a.x + 2, a.y + 2, 9, 9, Color(0, 153, 153, 255));
		}
		else
		{
...
{
	POINT a = GetAbsolutePos();
	Render::Text(a.x, a.y, Color(195, 195, 195, 255), Render::Fonts::MenuBold, Text.c_str());
}
...
void CGroupBox::Draw(bool hover)
{
	...
	Render::Line(a.x, a.y, a.x + 12, a.y, Color(230, 230, 230, 255));
	Render::Line(a.x + 15 + txtSize.right + 5, a.y, a.x + m_iWidth, a.y, Color(230, 230, 230, 255));
	Render::Line(a.x, a.y, a.x, a.y + m_iHeight, Color(230, 230, 230, 255));
	Render::Line(a.x, a.y + m_iHeight, a.x + m_iWidth, a.y + m_iHeight, Color(230, 230, 230, 255));
	Render::Line(a.x + m_iWidth, a.y, a.x + m_iWidth, a.y + m_iHeight, Color(230, 230, 230, 255));
}
...
void CSlider::Draw(bool hover)
{
	...
	Render::Clear(a.x + Location, a.y + 1, 4, 9, Color(255, 51, 153, 255));
	Render::Outline(a.x + Location, a.y + 1, 4, 9, Color(255, 51, 153, 255));

	char buffer[24];
	sprintf_s(buffer, "%.2f", Value);
	RECT txtSize = Render::GetTextSize(Render::Fonts::MenuBold, buffer);
	Render::Text(a.x + (m_iWidth / 2) - txtSize.right / 2, a.y + 10, Color(195, 195, 195, 255), Render::Fonts::MenuBold, buffer);
}
...
void CListBox::Draw(bool hover)
{
	...
	if (Items.size() > 0)
	{
		...
		for (int i = ScrollTop; (i < Items.size() && drawnItems < ItemsToDraw); i++)
		{
			...
			Render::Text(ItemRegion.left + 4, ItemRegion.top + 2, textColor, Render::Fonts::Menu, Items[i].c_str());
			drawnItems++;
		}
